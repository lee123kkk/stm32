...
// 변환시작: EXTI 11 발생할 때마다 start trigger 명령
...
{     	// ADC1: PA1(pin 41)
	...
	ADC1->CR2 |=  (2<<28);		// EXTEN[1:0]: ADC_ExternalTrigConvEdge_Enable(Falling Edge) //원본 코드
//        ADC1->CR2 |=  (1<<28);		// EXTEN[1:0]: ADC_ExternalTrigConvEdge_Enable(Risingl Edge) //수정된 코드

	//*****************//
	ADC1->CR2 |= (0x0F<<24);	// EXTSEL[3:0]: ADC_ExternalTrig (EXTI11)  //EXTEN을 1111로 설정
...
void _EXTI_Init(void)    //EXTI11(PH11,SW3)
{
	...
	EXTI->FTSR |= (1<<11);		// 0x000800, Falling Trigger Enable  (EXTI11:PH11)  //원본 코드
//	EXTI->RTSR |= (1<<11);		// 0x000800, Rising Trigger Enable  (EXTI11:PH11)  //수정된 코드
}
