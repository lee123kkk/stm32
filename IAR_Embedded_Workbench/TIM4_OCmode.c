...
int main(void)
{
	...
	while(1)
	{
		...
		switch(KEY_Scan())
		{
                        case SW1_PUSH : //SW1 //추가한 코드
                                  TIM4->CR1 |= (1<<0); //CNT enable //추가한 코드
                        break;

                        case SW2_PUSH : //SW12//추가한 코드
                                  TIM4->CR1 &= ~(1<<0); //CNT enable //추가한 코드
                        break;
...
//				TIM4->CCR1 = 1000; 	// 1ms ON //원본 코드
                                TIM4->CCR1 = 5000; 	// 1ms ON //수정된 코드
//	                        TIM4->CCER |= (1<<0);	//출력 가능 //수정된 코드
			break;

			case SW5_PUSH  : 	//SW5
				GPIOG->ODR &= 0x0F;	// LED4~7 OFF
				GPIOG->ODR |= 0x20;	// LED5 ON
//				TIM4->CCR1 = 4000; 	// 4ms ON //원본 코드
				TIM4->CCR1 = 20000; 	// 4ms ON //수정된 코드
//	                        TIM4->CCER &=~ (1<<0);	//출력  차단 //수정된 코드
...
//				TIM4->CCR1 = 8000;  	// 8ms ON  //원본 코드
				TIM4->CCR1 = 40000;  	// 8ms ON //수정된 코드
			break;	
		}
	}
}
...
// Time base 설정
	...
	// Setting the Period
//	TIM4->PSC = 84-1;		// Prescaler=84, 84MHz/84 = 1MHz (1us)  //원본 코드
	TIM4->PSC = 8400-1;		// Prescaler=84, 84MHz/8400 = 10K Hz (0.1ms)  //수정된 코드
//	TIM4->ARR = 10000-1;	// Auto reload  : 1us * 50K = 5(period) : 인터럽트주기나 출력신호의 주기 결정  //원본 코드
         TIM4->ARR = 50000-1;	// Auto reload  : 0.1ms * 10K = 10ms(period) : 인터럽트주기나 출력신호의 주기 결정   //t수정된 코드
//        TIM4->ARR = 25000-1;	// Auto reload  :    주기 25ms (출력 신호의 주기 50ms) //수정된 코드
//      TIM4->ARR = 50000-1;	// Auto reload  :    주기 50ms (출력 신호의 주기 100ms) //수정된 코드
...
// Output Compare 설정
	...
	TIM4->CCER |= (1<<0);	// CC1E=1: CC1 channel Output Enable            //원본 코드
				// OC1(TIM4_CH1) Active: 해당핀(100번)을 통해 신호출력
//	TIM4->CCER &= ~(1<<0);	// CC1E=0 CC1 channel Output disable            //수정된 코드
...
void _EXTI_Init(void)
{
	...
	SYSCFG->EXTICR[2] &= ~0x00FF; 	// EXTICR[2]의 EXTI8,9 값을 reset
	SYSCFG->EXTICR[2] |= 0x0077; 	// EXTI8,9에 대한 소스 입력은 GPIOH로 설정 (EXTICR3) 	
					// EXTI8 <- PH8, EXTI9 <- PH9 
