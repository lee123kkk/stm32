...
// 동작: OC1을 통한 PWM Pulse(DC motor 구동) 출력과 Encoder에 의한 모터회전수 측정, RPM 측정
//////////////////////////////////////////////////////////////////////
#include "stm32f4xx.h"
#include "GLCD.h"
...
uint16_t ENCODER, ENCODER_NEW, ENCODER_OLD;
uint32_t RPM;
uint8_t ENC[5];
...
void TIMER3_Init(void)
{
	...
	TIM3->ARR = 10000-1;	// Auto reload  0.1ms * 10000 = 1s
...
void TIM3_IRQHandler(void)  	// 1s Interrupt                                      //중간고사 대비 계산 공부할것
{
	TIM3->SR &= ~(1<<0);	// Interrupt flag Clear
	ENCODER_OLD= ENCODER_NEW;
	ENCODER_NEW = TIM4->CNT;	// TIM4 CNT 값(Encoder 펄스 수) 읽음
	if (ENCODER_NEW < ENCODER_OLD)
		RPM= 10000-(ENCODER_OLD-ENCODER_NEW);
	else
		RPM= ENCODER_NEW-ENCODER_OLD;// 현재 엔코더값 - 과거 엔코더 값
	RPM = RPM*60 /3;	// 엔코더 사양: 1 turn당 3 pulse 출력
	ENC[4]= RPM/10000;	// 10000 자리
	ENC[3]= RPM%10000/1000;	// 1000 자리
	ENC[2]= RPM%1000/100;	// 100 자리
	ENC[1]= RPM%100/10;	// 10 자리
	ENC[0]= RPM%10;		// 1 자리
	LCD_DisplayChar(4,5,ENC[4]+0x30);
	LCD_DisplayChar(4,6,ENC[3]+0x30);
	LCD_DisplayChar(4,7,ENC[2]+0x30);
	LCD_DisplayChar(4,8,ENC[1]+0x30);
	LCD_DisplayChar(4,9,ENC[0]+0x30);
...
// PD12을 입력설정하고 Alternate function(TIM4_CH1)으로 사용 선언
	GPIOD->MODER 	|= (2<<24);	// 0x00000000(MODER.(25,24)=0b00), GPIOD PIN12 intput Alternate function mode 					
