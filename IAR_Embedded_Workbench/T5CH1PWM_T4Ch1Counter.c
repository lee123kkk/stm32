...
// Time base Mode
	// Setting CR1 : 0x0000 
	TIM4->CR1 &= ~(1<<4);	// DIR=0(Up counter)(reset state)    //원본코드
//        TIM4->CR1 |= (1<<4);	// DIR=1(Down counter)(reset state)    //수정한 코드
	...
	TIM4->CR1 &= ~(3<<5); 	// CMS(Center-aligned mode Sel)=00 :Edge-aligned mode(reset state)   //원본 코드
//	TIM4->CR1 |= (3<<5); 	// CMS(Center-aligned mode Sel)=11      //수정된 코드

	// PSC, ARR
	TIM4->PSC = 1-1;	// Prescaler=1
	TIM4->ARR = 10000-1;	// Auto reload  :  count값 범위: 0~10000        //원본코드
 //       TIM4->ARR = 1000 ;                           //수정한 코드
...
// External Clock Mode 1
	...
	TIM4->CCER &= ~(1<<1);	// CC1P=0              //원본코드
	TIM4->CCER &= ~(1<<3);	// CC1NP=0          //원본코드

//       TIM4->CCER |= (1<<1);	// CC1P=1             // 수정한 코드
//	TIM4->CCER |= (1<<3);	// CC1NP=1          //수정한 코드


	// SMCR(Slave Mode Control Reg.) : External Clock Enable
	TIM4->SMCR |= (5<<4);	// TS(Trigger Selection)=0b101 :TI1FP1(Filtered Timer Input 1 출력신호)  //원본 코드
//	TIM4->SMCR |= (4<<4);	// TS(Trigger Selection)=0b101 :TI1_ED(TI1 Edge Detector출력신호)  //수정한 코드
