//////////////////////////////////////////////////////////////////////
// DMA를 이용한 USART(RX, Tx) 통신 
// USART1: TX pin: PA9, RX pin: PA10 
//    - DMA 방식 (interrupt 사용 안함)
// Rx: PC(Hyper terminal: Herculus)에서 전송한 데이터(USART1->DR)를  
//      DMA를 통해 메모리(USART_value[ ])에 보내어 저장함
// Tx: 메모리(Tx_ch[ ]) 데이터를 DMA를 통해 USART1->DR에 보내어 
//     PC(Hyper terminal: Herculus)로 전송함
//////////////////////////////////////////////////////////////////////
#include "stm32f4xx.h"
#include "GLCD.h"
...
uint8_t USART_value[2];  //Rx 시험용

#define SW0_PUSH        0xFE00  //PH8
char Tx_ch[4] = {0,};   //Tx 시험용
...
//		LCD_DisplayChar(3,0, USART_value[0]); 
		...
		switch(KEY_Scan())
		{	case SW0_PUSH : 	
				GPIOG->ODR &= ~0xFF;// All LED Off
 				GPIOG->ODR |= 0x01;// LED0 On
 				Tx_ch[0] = '1';
				Tx_ch[1] = '2';
				Tx_ch[2] = '3';
				Tx_ch[3] = '4';
				USART1->CR3 |= (1<<7);	// UART- DMA 송신설정 (DMA mode is enabled for TX)
				while( !(DMA2_Stream7->FCR & (5<<3)) ){ }	// FIFO status Check: FIFO is Full?
				USART1->CR3 &= ~(1<<7);	// UART- DMA 송신설정 (DMA mode is disabled for TX)
			break;
		}
	}
}
...
/*  DMA2 Stream2 CH4 를 USART1 Rx과 연결 */
	...
	DMA2_Stream2->PAR |= (uint32_t)&USART1->DR;    //Peripheral address - USART1 RDR Address
...
/*  DMA2 Stream7 CH4 를 USART1 Tx와 연결 */
	DMA2_Stream7->CR &= ~(7<<25);	//DMA2 Stream7 channel selection bits Clear
	DMA2_Stream7->CR |= (4<<25);		//DMA2 Stream7 channel 4 selected
	DMA2_Stream7->M0AR |= (uint32_t)Tx_ch;		//Memory address - Tx_ch[ ]
	DMA2_Stream7->PAR |= (uint32_t)&USART1->DR ;	//Peripheral address - USART1->DR (TDR)
	DMA2_Stream7->NDTR = 4;			//DMA_BufferSize = 4

	DMA2_Stream7->CR &= ~(3<<6);		//CR.DIR[ ] clear 
	DMA2_Stream7->CR |=  (1<<6);		//Data transfer direction : memory-to-Peripheral
	DMA2_Stream7->CR &= ~(1<<9); 		//Peripheral increment mode - Peripheral address pointer is fixed
	DMA2_Stream7->CR |= (1<<10);		//Memory increment mode - Memory address pointer is incremented after each data transferd

	DMA2_Stream7->CR &= ~(3<<11);	//Peripheral data size - byte(8bit)
	DMA2_Stream7->CR &= ~(3<<13);	//Memory data size - byte(8bit)  
	DMA2_Stream7->CR |= (1<<8);		//Circular mode enabled  
	DMA2_Stream7->CR |= (3<<16);		//Priority level - Medium, 여러개의 스트림을 사용할때 의미있음

	DMA2_Stream7->FCR |= (1<<2);		//Direct mode disabled (FIFO mode  enabled)
//	DMA2_Stream7->FCR &= ~(1<<2);	//Direct mode enabled (FIFO mode  disabled)
	DMA2_Stream7->FCR |= (3<<0);		//DMA_FIFO Threshold_Full(Not used in direct mode), FIFO가 Full 일때는 반드시 FIFO를 읽어야 함

	DMA2_Stream7->CR &= ~(3<<23);	//Memory burst transfer configuration - single transfer
	DMA2_Stream7->CR &= ~(3<<21);	//Peripheral burst transfer configuration - single transfer  
	DMA2_Stream7->CR |= (1<<0);		//DMA2_Stream7 enabled
