...
int main(void)
{
 ...
 /*                                                                                                
                                case SW6_PUSH : 	//SW6    오버런 삭제              //추가한 코드
                                overrun =0;                                                          //추가한 코드
                                case SW7_PUSH : 	//SW7         오버런 발생        //추가한 코드
                                overrun =1;                                                          //추가한 코드
*/
			break;
                }      
...
uint8_t Xcur, Ycur=0; //추가한 코드

void USART1_IRQHandler(void)	
{       
 /*      
        if(USART1->SR & USART_SR_PE)//0x0001                 //추가한 코드
             LCD_DisplayText(2,0, "Parity Error! ");                 //추가한 코드
        if(USART1->SR & USART_SR_ORE)//0x0008                 //추가한 코드
             LCD_DisplayText(3,0, "Overrun Error! ");                 //추가한 코드
        if(USART1->SR & USART_SR_FE)//0x0002                 //추가한 코드
            LCD_DisplayText(4,0, "Frame Error! ");                 //추가한 코드
        if(overrun ==0)                                                        //추가한 코드
        {                                                                             //추가한 코드
*/
...
//		LCD_DisplayChar(1,0,ch); 	// 수신된 문자를 LCD에 display   //원본 코드
                LCD_DisplayChar(Ycur,Xcur,ch); 	// 수신된 문자를 LCD에 display   //추가한 코드
                if(Xcur==15) {Xcur=0; Ycur++;}   //추가한 코드
...
void USART1_Init(void)
{
	...
	USART_BRR_Configuration(19200); // USART Baud rate Configuration  //원본 코드
//     USART_BRR_Configuration(9600); // USART Baud rate Configuration  //추가한 코드
    
//	USART1->CR1	&= ~(1<<12);	// USART_WordLength 8 Data bit  //원본 코드
//	USART1->CR1	&= ~(1<<10);	// NO USART_Parity  //원본코드

        USART1->CR1	|= (1<<12);	// USART_WordLength 9Data bit           //추가한 코드 
	USART1->CR1	|= (1<<10);	//  USART_Parity                      //추가한 코드 odd페리티일때 
        USART1->CR1	|= (1<<9);	                                                               //추가한 코드 odd페리티일때 
