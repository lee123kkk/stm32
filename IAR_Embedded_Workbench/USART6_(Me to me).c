...
// -RX INT, TX polling, 9600 baud-9 data bits-1 stop bit-EvenParity
...
// -SW4 push: “456” 송신
// (3) RX : 문자 수신
//-초기 커서 위치 지정: Xpos=0, Ypos=1
// -문자 수신시 현재커서 위치에 문자 표시하고, 커서위치update(Xpos++) -’0’ 수신 : 부저 1회
// -’1’ 수신 : 부저 2회
// -’2’ 수신 : 부저 3회
// - 커서가 LCD의 우측 모서리에 도착하면 줄을 변경하여문자표시(즉,Ypos++) 
//////////////////////////////////////////////////////////////////////
#include "stm32f4xx.h"
#include "GLCD.h"
		...
		if ( (USART6->SR & USART_SR_RXNE) ) 
                {
                        ...
                        SerialSendChar(ch); 		// PC에 수신된 문자를 Echo (Me-to-Me 테스트용)

                // PDF 요구사항: 수신 시 현재 커서 위치에 표시 [cite: 15]
                        LCD_DisplayChar(Ypos, Xpos, ch); 	// 수신된 문자를 LCD에 display

                // PDF 요구사항: 커서 위치 업데이트 (Xpos++) [cite: 15]
                Xpos++; 
                        if(Xpos == 16) // LCD 가로 폭에 맞게 수정 (15 또는 16)
                {
                    Xpos = 0; 
                    Ypos++; // PDF 요구사항: 줄 변경 (Ypos++) [cite: 19]
                }   

                // PDF 요구사항: 수신 문자에 따른 부저 동작 추가
                switch(KEY_Scan())
                {
                        case SW0_PUSH : 		//SW0
...
//		LCD_DisplayChar(1,0,ch); 	// 수신된 문자를 LCD에 display   //원본 코드
                LCD_DisplayChar(Ypos,Xpos,ch); 	// 수신된 문자를 LCD에 display   //추가한 코드
                if(Xpos==15) {Xpos=0; Ypos++;}   //추가한 코드
                if (Xpos > 18) 
		{	Xpos= 0;
			Ypos++;
		}
...
void USART6_Init(void)
{
	...
	GPIOA->MODER	|= (2<<2*6);	// GPIOC PIN6 Output Alternate function mode					
	GPIOA->OSPEEDR	|= (3<<2*6);	// GPIOC PIN6 Output speed (100MHz Very High speed)
	GPIOA->AFR[0]	|= (8<<24);	// Connect GPIOC pin6 to AF8(USART6)
    
	// USART6 : RX(PA7)
	GPIOA->MODER 	|= (2<<2*7);	// GPIOC PIN7 Output Alternate function mode
	GPIOA->OSPEEDR	|= (3<<2*7);	// GPIOC PIN7 Output speed (100MHz Very High speed
	GPIOA->AFR[0]	|= (8<<28);	// Connect GPIOC pin10 to AF8(USART6)

	RCC->APB2ENR	|= (1<<5);	// RCC_APB2ENR USART6 Enable

        USART_BRR_Configuration(9600); // USART Baud rate Configuration   

        USART6->CR1	|= (1<<12);	// USART_WordLength 9Data bit         
	USART6->CR1	|= (1<<10);	//  USART_Parity            
        USART6->CR1	|= (0<<9);          // even 페리티일때 
...
{ 
	...
	else  // USART6->CR1.OVER8 = 0 (16 oversampling)
