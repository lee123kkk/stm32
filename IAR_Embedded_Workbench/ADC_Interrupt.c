...
void ADC_IRQHandler(void)
{
	...
	Voltage = ADC_Value * (3.3 * 100) / 4095;   // 3.3 : 4095 =  Volatge : ADC_Value  //원본 코드(12bit)
                                                    // 100:  소수점아래 두자리까지 표시하기 위한 값  
//	Voltage = ADC_Value * (3.3 * 100) / 255;   // 3.3 : 255 =  Volatge : ADC_Value  //수정한 코드(8bit)
	...
	ADC1->CR2 |= ADC_CR2_SWSTART; 	// 0x40000000 (1<<30)    //인터럽트 발생 시 전압 측정
}
...
{	// ADC1: PA1(pin 41)
	...
	ADC1->CR1 &= ~(3<<24);		//  00       12bit Resolution            //원본 코드
//        ADC1->CR1 |= (2<<24);		//  10        8bit Resolution            //수정한 코드
	ADC1->CR1 &= ~(1<<8);		// SCAN=0 : ADC_ScanCovMode Disable
	ADC1->CR1 |=  (1<<5);		// EOCIE=1: Interrupt enable for EOC   //폴링에는 없는 부분

	ADC1->CR2 &= ~(1<<1);		// CONT=0: ADC_Continuous ConvMode Disable   //원본코드
//     ADC1->CR2 |= (1<<1);	   //CONT=1: //수정한 코드 (언속 실행)
