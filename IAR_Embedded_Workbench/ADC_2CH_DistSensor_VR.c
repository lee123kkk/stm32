...
{
	 if (ADC1->SR && ADC_SR_EOC == ADC_SR_EOC) // ADC1 EOC int   //ADC1때문에 발생한 인터럽트인지 확인
	{
		ADC1->SR &= ~(1<<1);	// EOC flag clear
  
		ADC_Value = ADC1->DR;		// Reading ADC1 result
 	...
 	if (ADC2->SR && ADC_SR_EOC == ADC_SR_EOC) // ADC2 EOC int  //ADC2때문에 발생한 인터럽트인지 확인
	{
...
void _ADC_Init(void)                  //채널을 여러개 쓰면 식이 길어지지만 단순함 (채널 갯수가 3개 이하이면 가능함)
{
// ADC1: PA1(pin 41)
	...
	ADC1->CR2 &= ~(1<<1);		// CONT=0: ADC_Continuous ConvMode Disable   //연속모드 사용안함
