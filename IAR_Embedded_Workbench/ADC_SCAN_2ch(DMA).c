...
//	ADC->CCR |= 0x00000F00;	// ADC_TwoSamplingDelay_20Cycles
	...
	ADC1->CR1 |= 0x00000100;	// ADC_ScanCovMode Enable (SCAN=1)        //스캔 모드 설정
	ADC1->CR2 |= 0x00000002;	// ADC_ContinuousConvMode ENABLE (CONT=1)  //연속 실행
	...
	ADC1->CR2 |= 0x00000200;	// DMA requests are issued as long as data are converted and DMA=1	//ADC 컨버터에 DMA 사용
				// for single ADC mode
	/* Enable ADC1 DMA */
	ADC1->CR2 |= 0x00000100;	// DMA mode enabled  (DMA=1)  //ADC 컨버터에 DMA 사용
...
void DMAInit(void)
{
	...
	DMA2_Stream0->CR &= ~(7<<25);	//DMA2 Stream0 channel 0 selected   //채널 선택                                                 //중요함

	// ADC1->DR(Peripheral) ==> ADC_vlaue(Memory)
	DMA2_Stream0->PAR |= (uint32_t)&ADC1->DR;	   //Peripheral address - ADC1->DR(Regular data) Address  //&:주소 인자  //중요함
	DMA2_Stream0->M0AR |= (uint32_t)ADC_value; //Memory address - ADC_Value address                                                         //중요함
	DMA2_Stream0->CR &= ~(3<<6);		  //Data transfer direction : Peripheral-to-memory (P=>M)  //방향 설정
	DMA2_Stream0->NDTR = 2;			  //DMA_BufferSize = 2 (ADC_Value[2])   //데이터 수
        //ADC_value[0]의 주소값이 1000번지이고 2비트 일때 ADC_value[1]의 주소는 1001번지
