...
void TIMER3_Init(void);  // Timer3 초기화  //원본코드
void TIMER5_Init(void);  // Timer5 초기화  //추가한 코드
...
int main(void)
{
	...
	TIMER3_Init();		// 범용타이머(TIM3) 초기화 : up counting mode    //원본코드
//        TIMER5_Init();		// 범용타이머(TIM3) 초기화         //추가한 코드
	while(1)
	{
		switch(KEY_Scan())
		{
...
// Setting CR1 : 0x0000 
//	TIM3->CR1 &= ~(1<<4);	// DIR=0(Up counter)(reset state)      //원본 코드  (TIM3의 DIR(4번핀)의 값이 0)
    TIM3->CR1 |= (1<<4);	// DIR=0(down  counter)(reset state) //수정한 코드     (TIM3의 DIR(4번핀)의 값이 1)   
...
// Deciding the Period
	TIM3->PSC = 8400-1;	// Prescaler 84,000,000Hz/8400 = 10,000 Hz (0.1ms)  (1~65536) / /원본코드
//      TIM3->PSC = 16800-1;	// Prescaler 84,000,000Hz/16800 = 5,000 Hz (0.2ms) / /수정된 코드
//         TIM3->PSC = 42000-1       ;// Prescaler 84,000,000Hz/42000 = 2,000 Hz (0.5ms) / /수정된 코드


//	TIM3->ARR = 10-1;		// Auto reload  0.1ms * 10 = 1ms (원래 코드 부분)
//     TIM3->ARR = 40-1;		// Auto reload  0.1ms * 40 = 4ms //수정된 코드
//      TIM3->ARR = 5000-1;	    //추가한 코드  0.1ms *5000=500ms=0.5s
//      TIM3->ARR = 100000-1;	 //추가한 코드 65535를 넘는 값이므로 정상적으로 작동하지 않음
//      TIM3->ARR = 50000-1;           //추가한 코드  0.2 & 50000 = 10s (psc=16800일떄)
     TIM3->ARR = 1000-1;	 //추가한 코드 
...
// Setting an UI(UEV) Interrupt 
	...
	TIM3->CR1 |= (1<<0);	// Enable the Tim3 Counter (clock enable)                       클락 동작 시작
}


/*
void TIMER5_Init(void)   //추가한 코드
{
// Enable Timer CLK 
...
// Setting CR1 : 0x0000 
	...
	TIM5->CR1 &= ~(1<<2);	// URS=0(Update Request Source  Selection): By one of following events
				...
				//   - Update Generation through the slave mode controller 
				// URS=1 : Only Update Interrupt generated By Counter Overflow/Underflow,
	TIM5->CR1 &= ~(1<<5);	// OPM=0(The counter is NOT stopped at update event) (reset state)
	TIM5->CR1 &= ~(5<<7);	// ARPE=0(ARR is NOT buffered) (reset state)
	TIM5->CR1 &= ~(5<<8); 	// CKD(Clock division)=00(reset state)
	TIM5->CR1 &= ~(5<<5); 	// CMS(Center-aligned mode Sel)=00 (Edge-aligned mode) (reset state)
				// Center-aligned mode: The counter counts UP and DOWN alternatively

// Deciding the Period

        TIM5->PSC = 42000-1       ;// Prescaler 84,000,000Hz/42000 = 2,000 Hz (0.5ms) / /수정된 코드
        TIM5->ARR = 1000-1;	 //추가한 코드 
...
// Setting an UI(UEV) Interrupt 
	NVIC->ISER[1] |= (1<< 50-32); 	// Enable Timer5global Interrupt
 	TIM5->DIER |= (1<<0);	// Enable the Tim3 Update interrupt

	TIM5->CR1 |= (1<<0);	// Enable the Tim3 Counter (clock enable)                       클락 동작 시작
}

*/

unsigned char s100m=0, s1=0, s10=0;
...
*/
        s100m++;                             //추가한 코드
        if(s100m>=10) {s100m= 0;  s10++; if(s10>=10) {s10=0; s1++; if(s1>=6){s1=0;} }} //추가한 코드
        LCD_DisplayChar(5,9,s100m + 0X30);   //추가한 코드
        LCD_DisplayChar(5,7,s10 + 0X30);   //추가한 코드
        LCD_DisplayChar(5,6,s1 + 0X30);   //추가한 코드

//        GPIOG->ODR ^= 0x08;	//추가한 코드
}
...
void DisplayInitScreen(void)
{
	...
	LCD_DisplayText(4,0,"LED7 Off");
        ...
        LCD_DisplayText(5,0,"Watch 00:0");  //추가한 코드
